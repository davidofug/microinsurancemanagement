...
export default function Pagination({pages, currentClients, sortedEmployees, entries}){
    ...
    return (
        <div className="clearfix">
            ...
            <ul className="pagination">
                ...
                <li className={`${currentButton === 1 ? 'page-item disabled' : 'page-item'}`}>
                    <button onClick = {() => setCurrentButton( prev => prev === 1 ? prev : prev-1 )}>{'<'}</button>
                </li>
                ...
                <li className={`${currentButton === numOfPages.length ? 'page-item disabled' : 'page-item'}`}>
                    <button onClick={() => setCurrentButton(next => next === numOfPages.length ? next : next + 1)}>{'>'}</button>
                </li>
                <li className={`${currentButton === numOfPages.length ? 'page-item disabled' : 'page-item'}`}>
                    <button onClick={() => setCurrentButton(numOfPages.length)}>{'>>'}</button>
                </li>
            </ul>
        </div>
    )
