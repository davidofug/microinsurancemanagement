...
export default function Mtp() {
  useEffect(() => { document.title = "Britam - Transit"; getMTP()}, []);
  ...
  const getMTP = async () => {
    ...
    const mtpPolicies = policiesArray.filter(policy => policy.category === 'transit')
        ...
        {shownPolicies.length > 0
         ...
         :
          <div className="no-table-data">
              <i><ImFilesEmpty /></i>
              <h4>No match</h4>
              <p>There is no current match for Transit sticker</p>
          </div>
        }
