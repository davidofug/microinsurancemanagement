  ...
  const getWindscreen = async () => {
    ...
    if(authClaims.supervisor){
      ...
      listUsers().then(({data}) => {
        ...
        const supervisorWindscreenPolicies = windscreenPolicies.filter(policy => usersUnderSupervisor.includes(policy.added_by_uid))
        supervisorWindscreenPolicies.length === 0 ? setPolicies(null) : setPolicies(supervisorWindscreenPolicies)
      })
    }
    ...
    if(authClaims.admin){
      ...
      listUsers().then(({data}) => {
        ...
        const AdminWindscreenPolicies = windscreenPolicies.filter(policy => usersUnderAdmin.includes(policy.added_by_uid))
        AdminWindscreenPolicies.length === 0 ? setPolicies(null) : setPolicies(AdminWindscreenPolicies)
      })
    }
